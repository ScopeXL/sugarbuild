...
module.exports = {
    ...
    install: function() {
        ...
        if (!this.getConfig('installSugar')) {
            ...
            if (this.getConfig('enableBuildSchedule')) {
                installUrl = this.getConfig('baseWebUrl') + '/schedule/' + this.getConfig('flavor') +
    ...
    preImport: function() {
        ...
        if (!_.isEmpty(this.getConfig('importDumpFile'))) {
            ...
            sqlDumpLocation = this.getConfig('importHost') + '/data/' + this.getConfig('importBranch');
            this.log(
        ...
        /*if (_.isEqual(this.getConfig('importDumpFile').substring(0, 7), 'http://')) {
            ...
            http.get(sqlDumpLocation, function(res) {
                ...
                res.on('end', function() {
                    ...
                    fs.writeFile(tmpDumpLocation, sqlDumpData, 'utf8', function(err) {
                        if (err) {
                            deferred.reject(err);
                        }
