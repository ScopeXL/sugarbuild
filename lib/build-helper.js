...
module.exports = {
    ...
    setConfig: function(key, val) {
        ...
        socket.emit('config:set:complete', {
            setting: key,
            value: val
        });
    },
    ...
    log: function() {
        ...
        if (args.length % 2 === 0) {
            ...
            socket.emit('console:message', logMsg);
        } else {
    ...
    watchChanges: function() {
        ...
        // When Sidecar is being rebuilt prevent from triggering multiple times
        var sidecar_building = false;
        // When CSS is built prevent from triggering multiple times
        var css_building = false;
                            ...
                            Q.when($this.copyFile(filename)).then(function() {
                                ...
                                switch (ext) {
                                    ...
                                    case '.less':
                                        ...
                                        break;
                                    case '.js':
                                        // Rebuild sidecar if needed
                                        if (filename.indexOf('include/javascript') > -1 ||
                                            filename.indexOf('sidecar/') > -1)
                                        {
                                            var jsDir = path.join(
                                                $this.getConfig('outputDir'),
                                                $this.getConfig('flavor'),
                                            ...
                                            if (sidecar_building) {
                                                // Sidecar is being built
                                                return;
                                            }

                                            sidecar_building = true;

                                            fs.rmrf(jsDir, function(err) {
                                                if (err) {
                                                    $this.log(err, 'red');
                                                } else {
                                                    var sidecarCmd = 'cd ' +
                                                        path.join(
                                                            $this.getConfig('outputDir'),
                                                            $this.getConfig('flavor'), 'sidecar'
                                                        ) + ';' +
                                                        'gulp build:min';

                                                    $this.log('Rebuilding Sidecar...', 'yellow');

                                                    setTimeout(function() {
                                                        Q.when($this.runCommand(sidecarCmd)).then(function(stdout, stderr) {
                                                            // Reset sidecar build status
                                                            sidecar_building = false;
                                                            $this.log('Sidecar Rebuilt', 'green');
                                                        });
                                                    }, 1500);
                                                }
                                            });
    ...
    emailSettings: function() {
        var deferred = Q.defer(),
            cmd = `mysql -u root -e "INSERT INTO ${config.sugar_config_si.setup_db_database_name}.outbound_email (
                id,
                name,
                type,
                user_id,
                ...
                '13776dd6-9a0e-11e6-9e86-a45e60e495c4',
                'system',
                'system-override',
                1,
                'SMTP',
                ...
                ${config.emailSettings.smtpAuthentication},
                0
            )";`;

            cmd += `mysql -u root -e "UPDATE ${config.sugar_config_si.setup_db_database_name}.outbound_email SET
                name='system',
                user_id=1,
                mail_sendtype='SMTP',
                mail_smtptype='other',
                mail_smtpserver='${config.emailSettings.smtpServer}',
                mail_smtpport=${config.emailSettings.smtpPort},
                mail_smtpuser='${config.emailSettings.username}',
                mail_smtppass='${config.emailSettings.password}',
                mail_smtpauth_req=${config.emailSettings.smtpAuthentication},
                mail_smtpssl=0
            WHERE type='system'";`;

            cmd += `mysql -u root -e "INSERT INTO ${config.sugar_config_si.setup_db_database_name}.email_addresses (
                id,
                email_address,
                email_address_caps
            ) VALUES (
                '0c4bcbdc-9c5d-11e6-b8da-a45e60e495c5',
                '${config.emailSettings.adminEmailAddress}',
                '${config.emailSettings.adminEmailAddressUpper}'
            )";`;

            cmd += `mysql -u root -e "INSERT INTO ${config.sugar_config_si.setup_db_database_name}.email_addr_bean_rel (
                id,
                email_address_id,
                bean_id,
                bean_module,
                ...
                '0c4bbab6-9c5d-11e6-9c94-a45e60e495c5',
                '0c4bcbdc-9c5d-11e6-b8da-a45e60e495c5',
                1,
                'Users',
                1
            ...
            // cmd = 'mysql -u root -e "INSERT INTO ' + config.sugar_config_si.setup_db_database_name + '.outbound_email (id, name, type, user_id, mail_sendtype, mail_smtptype, mail_smtpserver, mail_smtpport, mail_smtpuser, mail_smtppass, mail_smtpauth_req, mail_smtpssl) ' +
            //         'VALUES (\'13776dd6-9a0e-11e6-9e86-a45e60e495c4\', \'system\', \'system-override\', 1, \'SMTP\', \'other\', \'localhost\', 1025, \'\', \'\', 0, 0)";' +
            //     'mysql -u root -e "UPDATE ' + config.sugar_config_si.setup_db_database_name + '.outbound_email ' +
            //         'SET name=\'system\', user_id=1, mail_sendtype=\'SMTP\', mail_smtptype=\'other\', mail_smtpserver=\'localhost\', mail_smtpport=1025, mail_smtpuser=\'\', mail_smtppass=\'\', mail_smtpauth_req=0, mail_smtpssl=0 WHERE type=\'system\'";' +
            //     'mysql -u root -e "INSERT INTO ' + config.sugar_config_si.setup_db_database_name + '.email_addresses (id, email_address, email_address_caps) ' +
            //         'VALUES (\'0c4bcbdc-9c5d-11e6-b8da-a45e60e495c5\', \'a@a.com\', \'A@A.COM\')";' +
            //     'mysql -u root -e "INSERT INTO ' + config.sugar_config_si.setup_db_database_name + '.email_addr_bean_rel (id, email_address_id, bean_id, bean_module, primary_address) ' +
            //         'VALUES (\'0c4bbab6-9c5d-11e6-9c94-a45e60e495c5\', \'0c4bcbdc-9c5d-11e6-b8da-a45e60e495c5\', 1, \'Users\', 1)"';
    ...
    // Start Mailcatcher
    mailcatcher: function(options) {
        var deferred = Q.defer();
        var cmd = 'mailcatcher -f';

        Q.when(this.runCommand(cmd, options)).then(function(stdout, stderr) {
            deferred.resolve();
        });
    ...
    full: function(callback) {
        ...
        // set is building to stop watched files from copying
        this.building = true;
        ...
        socket.emit('build:start:complete');
    ...
    // tail the sugarcrm.log file for output
    tailLog: function() {
        if (!_.isUndefined(tailedLog)) {
            tailedLog.watch();
            return;
        }

        var $this = this;
        tailedLog = new Tail('/Users/eroesch/Sites/ent/sugarcrm.log');

        tailedLog.on('line', function(data) {
            socket.emit('log:data', data);
        });

        tailedLog.on('error', function(error) {
            $this.log(error, 'red');
        });

        this.log('Watching sugarcrm.log for messages...', 'gray');
    },
    // stop tailing the sugarcrm.log file
    stopTailLog: function() {
        if (!_.isUndefined(tailedLog)) {
            tailedLog.unwatch();
        }
    },
    // execute a background command
    runCommand: function(cmd, options) {
        var deferred = Q.defer(),
            $this = this,
            stdout = '',
            stderr = '',
        ...
        bgCmd.stdout.on('data', function(data) {
            ...
            if (isVerbose || (!_.isUndefined(options) && options.logOutput)) {
